...
- `thermion_sakura`: scene generation, geometry, shaders, post-processing, and fidelity tools.
- `thermion_sakura_native`: headless native renderer used for fidelity work.
- `sakura_explorer`: interactive Flutter desktop application.
...
Materials are embedded in `thermion_sakura/lib/src/materials_gen.dart`. Compile them with `matc` from the Filament version in `filament.version` (currently v1.75.0).
...
```sh
dart format thermion_sakura/lib/src/materials_gen.dart
dart analyze thermion_sakura/lib/src/materials_gen.dart
```
...
```sh
cd thermion_sakura_native
dart pub get
cd ../thermion_sakura
python3 tool/render_fidelity_suite.py --only yonchome_hall
```

See `thermion_sakura/README.md` for the full fidelity workflow and implementation notes.

## Validation status

- `thermion_sakura` and `thermion_sakura_native` resolve their pinned Git dependencies and pass scoped Dart analysis.
- `sakura_explorer` resolves dependencies and completes Flutter analysis with informational lints only.

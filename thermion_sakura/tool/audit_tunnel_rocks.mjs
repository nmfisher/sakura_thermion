...
const meshPrefix = process.env.MESH_PREFIX || 'hillRock';
...
try {
  ...
  const rows = await page.evaluate(async (meshPrefix) => {
    ...
    player.pos.set(91, world.heightAt(91, 7.2), 7.2);
    ...
    scene.traverse((object) => {
      if (!object.isInstancedMesh ||
          (meshPrefix !== '*' && !object.name.startsWith(meshPrefix))) return;
      for (let i = 0; i < object.count; i++) {
        object.getMatrixAt(i, local);
        worldMatrix.multiplyMatrices(object.matrixWorld, local);
        worldMatrix.decompose(position, rotation, scale);
        const screen = position.clone().project(camera);
        const screenX = (screen.x * .5 + .5) * 1600;
        const screenY = (-screen.y * .5 + .5) * 900;
        if (screen.z < -1 || screen.z > 1 || screenX < 0 || screenX > 1600 ||
            screenY < 0 || screenY > 900) continue;
        const flat = flatAt(position);
        if (flat.x < 90 || flat.x > 125 || flat.z < 6 || flat.z > 20) continue;
        ...
        out.push({
          ...
          materialColor: object.material?.color?.getHex() ?? 0,
          tone: meshPrefix === '*' ? 0 : Number(object.name.slice(meshPrefix.length)),
    ...
    return out.sort((a, b) => a.screenX - b.screenX || a.screenY - b.screenY);
  }, meshPrefix);
  const output = process.env.OUT ||
    `/tmp/sakura-object-parity/east-tunnel-${meshPrefix}.json`;

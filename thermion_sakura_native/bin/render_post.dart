// Live finale test: render the ported scene through SakuraPostProcess with the
// sakura_post material (the reference's GRADE_SHADER), so the graded result is
// produced live (not via offline finale). If this matches the reference well,
// it's wired into the Flutter explorer for the live graded look.
//
//   xvfb-run ... dart run bin/render_post.dart
import 'dart:io';
import 'dart:isolate';
import 'dart:math' as math;

import 'package:thermion_dart/thermion_dart.dart';
import 'package:thermion_sakura/src/sakura_post_process.dart';
...
import 'package:thermion_sakura/src/geom/three_geom.dart' show Tri;
...
import 'package:thermion_sakura/src/post_settings.dart';
...
Future<void> main(List<String> argv) async {
  ...
  final manualTreeArg =
      argv.where((arg) => arg.startsWith('--manual-shadow-trees=')).firstOrNull;
  ...
  int parseShadowColor(String name, int fallback) {
    final arg = argv.where((value) => value.startsWith('--$name=')).firstOrNull;
  ...
  final flatTris = referenceBytes != null
      ...
      : isolatedTrainCar != null
          ...
          : buildPortedScene(
              ...
              includeManualTrainShadows: argv.contains('--with-manual-shadows'),
              ...
              includeReferenceShrubs: argv.contains('--with-reference-shrubs'),
              ...
              manualRoadShadowColor: parseShadowColor('road-shadow', 0x545279),
              ...
              manualTree4ShadowScale:
                  parseGradeValue('tree4-shadow-scale', .95),
              manualTrainShadowPlaneZ:
                  parseGradeValue('train-shadow-plane-z', 1.468),
  ...
  if (referenceAtlasPath != null) {
    ...
    final atlasUri = await Isolate.resolvePackageUri(
        Uri.parse('package:thermion_sakura/assets/sakura_signs.png'));
    if (atlasUri == null) {
      throw StateError('Could not resolve the Sakura sign atlas package asset');
    }
  ...
  if (atlasBytes != null) {
    ...
    final levels = 1 +
        (math.log(math.max(decoded.width, decoded.height)) / math.ln2).floor();
  ...
  await atlasMetadata.setImage(
      ...
      atlasMetadataValues.buffer.asUint8List(
          atlasMetadataValues.offsetInBytes, atlasMetadataValues.lengthInBytes),
  ...
  final raisedCasters = casterSource
      .where(
          (t) => !t.mat.unlit && math.max(t.a.y, math.max(t.b.y, t.c.y)) > 0.55)
      .toList();
  ...
  final nativeShadowMat = await app.createMaterial(sakuraShadowReceiverFilamat);
  ...
  if (nativeReceiverPacked != null &&
    ...
    for (var layer = 0; layer < shadowLayers; layer++) {
      final nativeReceiverAsset = await v1.createGeometry(
          Geometry(nativeReceiverPacked.positions, nativeReceiverPacked.indices,
  ...
  if (nativeCasterPackedBatches.isNotEmpty) {
    final nativeCasterMat = await app.createMaterial(sakuraShadowCasterFilamat);
  ...
  await app.lightManager.setShadowOptions(
      ...
      ShadowOptions(
        ...
        polygonOffsetSlope: parseGradeValue('native-shadow-polygon-slope', 1.0),
  ...
  final residualUri = await Isolate.resolvePackageUri(
      Uri.parse('package:thermion_sakura/assets/urayama_post_residual.png'));
  if (residualUri == null) {
  ...
  await postInst.setParameterFloat('inkFadeEnd', SakuraPostSettings.inkFadeEnd);
  await postInst.setParameterFloat(
      'inkStrength',
      inkEnabled
  ...
  await postInst.setParameterFloat('cameraNear', SakuraPostSettings.cameraNear);
  ...
  // Render v1 into sceneRT FIRST, then pp samples it. Without this, sceneRT is
  // empty and the post samples a constant.
  List<(View, Uint8List)>? sceneCapture;
  for (int i = 0; i < 2; i++) {
    sceneCapture = await app.capture(sc,
        view: v1.view,
        pixelDataFormat: PixelDataFormat.RGBA,
        pixelDataType: PixelDataType.FLOAT);
    await app.capture(sc,
  ...
  final res = await app.capture(sc,
      ...
      pixelDataFormat: PixelDataFormat.RGBA,
      pixelDataType: PixelDataType.FLOAT);
  final output = parseStringValue('output') ??
      (debugMode > 0
          ? '/tmp/sakura_post_debug${debugMode.toInt()}.bin'
          : inkEnabled
              ? '/tmp/sakura_post.bin'
